...
{
	if ( $stmt = $mysqli->prepare("UPDATE ".$tablename." SET ".$colname." = ? WHERE EmployeeID = ?"))
	{
...
{
	if ( $stmt = $mysqli->prepare("UPDATE ".$tablename." SET ".$colname." = ? WHERE PromoID = ?"))
	{
		$stmt->bind_param("si",$value, $id);
		$return = $stmt->execute();
		$stmt->close();
...
{
	if ( $stmt = $mysqli->prepare("UPDATE ".$tablename." SET ".$colname." = ? WHERE ShipmentID = ?"))
	{
		$stmt->bind_param("si",$value, $id);
		$return = $stmt->execute();
		$stmt->close();
...
{
	if ( $stmt = $mysqli->prepare("UPDATE ".$tablename." SET ".$colname." = ? WHERE SupplierID = ?"))
	{
		$stmt->bind_param("si",$value, $id);
		$return = $stmt->execute();
		$stmt->close();
